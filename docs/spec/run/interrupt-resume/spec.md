...
已有解析、handler、消息保存和终态函数作为现有依赖，保持实现与签名；工具仅调整选项数量约束。
具体实施范围见当前 plan.md；前端分页不改变恢复入口及前后端的参数与事件协议。
...
- 每道问题提供 1～3 个选项，优先给出 3 个明确且有区分度的选项；合理选项不足 3 个时不凑数。
  `HumanQuestion.options` 在工具输入 Schema 中声明 minItems=1、maxItems=3，工具说明同步
  提示上限；超过 3 个或为空的选项列表在调用校验阶段拒绝，不进入 interrupt。
  前端不截断选项，不丢弃已持久化的待回答问题；数量限制针对新生成的工具调用。
...
交互卡片只挂载在 ChatView 底部输入区内，紧邻文字输入框上方，不挂载在 messageScroller
或消息列表内，不随历史消息滚动。存在 pendingInteraction 时，即使尚无历史消息，输入区
也保持底部停靠。低高度视口中允许输入区自身滚动，确保问题和输入框可达。
组件在同一个卡片容器内按 questions 分页，一页显示一道问题及其全部单选项，选项各自呈现为
可点击的小卡片；不把多道问题纵向展开成长列表。选项展示 label、提交 value。
显示当前题号和总题数；选择选项后保存该题 answers，并自动切换到下一题。容器提供左右
上一题/下一题按钮，允许未回答时浏览其他题目，返回时保留并回显已有选择；首尾不循环。
最后一题选择后停留，全部问题作答后才允许点击 Continue，一次提交完整 answers；
翻页和选择本身不调用 Resume API。提交期间禁用选择、切换和重复提交；失败保留答案供重试。
切换只在容器内部执行短暂的水平位移与淡入淡出，尊重 prefers-reduced-motion；
长问题或较多选项仅在受限高度的内容区滚动，题号、导航及提交操作保持可达。
左右按钮可通过键盘操作；切换后焦点落在当前问题内，不留在已隐藏的选项上，
不覆盖 radio 原生方向键操作。父 Run 变化时清空答案并回到第一题。
选项的隐藏原生 radio 必须定位在对应 label 内；鼠标点击和键盘聚焦只允许问题列表的正常滚动，
不得使外层布局容器滚动而将整个聊天区域及输入框移出视口。验证需使用真实鼠标或键盘事件，
并检查容器位置，不能仅以 `input.click()` 或 DOM 文本存在作为可见性证据。
