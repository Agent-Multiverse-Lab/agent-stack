...
| ES-013 | RUN-ES-008/009 | `web/src/composables/useChat.ts`, `web/src/views/ChatView.vue`, `web/src/components/chat/loading/ChatThinkingGroupComponent.vue` | 归并并渲染消息、Run 状态、Agent state 和工具执行事件；Thinking Group 直接复用于 Thread Detail 读取和 Run 等待；有效 Agent tool 状态在 Thinking 后方同级展示，并跨正文输出与当前页面终态收口保留 |
...
- Thinking Group 只在当前 Run 活跃、尚无可见 Assistant 文本且未等待用户交互时显示，并在卸载时清理计时器。
- 当前 Run 的有效 Agent tool 状态紧随 Thinking 同级展示；空状态不渲染，正文输出和当前页面终态收口不删除已有 Tool 组件。
