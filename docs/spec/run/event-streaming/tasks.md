...
| ES-002 | RUN-ES-002/004 | `server/service/agent_run_service.py` | 按 envelope 读取进度、判断 end 并生成 DB 终态兜底 |
...
- SSE 在收到 `event_type=end` 后结束，公开 `data.type=end` 契约保持不变。
- DB 终态兜底事件不会重复写入 Redis。
