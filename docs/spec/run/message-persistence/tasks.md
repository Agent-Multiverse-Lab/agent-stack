...
| MP-004 | RUN-MP-003/004/005/006 | `server/service/thread_service.py` | 显式接收顶层 Run ID，从 checkpoint 读取 Message，按 `type` 调用两个 save 函数 |
...
- `ToolCall` 不再包含或接收排序值；
- AI Message 的 `id/name/args` 通过 `ConversationRepository.add_tool_call`
  写入一个 ToolCall；
...
- 两条 AI Message 都通过 `Message.agent_run_id` 关联当前 Run，
  遍历结束后仅在当前 Run 和最后一条 AI Message 都存在时调用一次
  `set_output_message`；
