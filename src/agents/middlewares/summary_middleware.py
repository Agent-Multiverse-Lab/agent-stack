...
DEFAULT_SUMMARY_PROMPT = """你是对话上下文压缩助手。
...
_TOOL_RESULT_SAVED_MARKER = "s2c_tool_result_saved"
...
_SUMMARY_COMPRESSION_STATE: ContextVar[dict[str, bool] | None] = ContextVar(
    "s2c_summary_compression_state",
    default=None,
)


class S2CSummarizationMiddleware(SummarizationMiddleware):
    ...
    def _compact_messages(self, messages: list[AnyMessage]) -> list[AnyMessage]:
        ...
        for message in messages:
            ...
            elif isinstance(message, ToolMessage) and getattr(message, "additional_kwargs", {}).get(_TOOL_RESULT_SAVED_MARKER) is not True and _should_offload_tool_message(message, self.tool_result_offload_token_limit):
    ...
    def _report_offload_result(file_path: str | None, failed_media: int) -> None:
        if file_path is None:
            message = "Offloading conversation history to backend failed during summarization. Older messages will not be recoverable."
...
def create_summary_middleware(
    ...
    *,
    ...
    return S2CSummarizationMiddleware(**middleware_kwargs)


def create_summary_middleware_from_context(
    context,
    *,
    backend,
) -> S2CSummarizationMiddleware:
    ...
    return create_summary_middleware(
        ...
        summary_prompt=getattr(context, "summary_prompt", None) or DEFAULT_SUMMARY_PROMPT,
...
def _emit_compression(status: str, **extra: Any) -> None:
    try:
        ...
        return
    writer({"type": "s2c.context_compression", "status": status, **extra})
...
def _extract_text_content(content: Any) -> str:
    ...
    if isinstance(content, list):
        parts = [item if isinstance(item, str) else item["text"] for item in content if isinstance(item, str) or isinstance(item, dict) and isinstance(item.get("text"), str)]
...
def _search_result_record(tool_name: str, result: Any) -> tuple[dict[str, Any], str]:
    ...
    keys = ("id", "kb_id", "file_id", "title", "source", "score", "distance") if tool_name == "query_kb" else ("title", "url", "site_name", "publish_time", "score")
...
def _truncate_provider_tool_calls(
    ...
    for raw_call in raw_tool_calls:
        ...
        if not isinstance(function, dict) or function.get("name") not in {"write_file", "edit_file"} or not isinstance(arguments, str) or len(arguments) <= max_length:
