...
class InputMessageServiceTest(unittest.TestCase):
    def test_attachment_file_ids_are_forwarded_without_content_expansion(self):
        message = build_agent_input_msg(
            query="请分析",
            msg_metadata={"attachment_file_ids": [FILE_ID], "source": "web"},
        )

        self.assertEqual("请分析", message.langchain_msg.content)
        self.assertEqual(
            {"attachment_file_ids": [FILE_ID], "source": "web"},
            message.langchain_msg.additional_kwargs,
        )

    def test_file_only_input_is_valid(self):
        message = build_agent_input_msg(
            msg_metadata={"attachment_file_ids": [FILE_ID]},
        )
        ...
        self.assertEqual(
            [FILE_ID],
            message.langchain_msg.additional_kwargs["attachment_file_ids"],
        )

    def test_attachment_file_ids_must_be_a_list(self):
